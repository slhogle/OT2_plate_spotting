...
def aspirate_spot(pipette, well_source, agar_dest, spot_vol=2,
    ...
    # try and knock off remaining droplets
...
def run(protocol: protocol_api.ProtocolContext):
    ...
    for p_dl, p_ag in zip(dilution_plates, agar_plates):
        ...
        for col in range(1, 8):
            ...
            transfer(p20_mult, 10, 1, p_dl[w], p_dl[w_offset])
        # do last transfers where some are 2-fold dilution
        transfer(p20_mult, 50, 1, p_dl['A7'], p_dl['A8'])
        transfer(p20_mult, 10, 1, p_dl['A8'], p_dl['A9'])
        transfer(p20_mult, 50, 1, p_dl['A9'], p_dl['A10'])
        transfer(p20_mult, 10, 1, p_dl['A10'], p_dl['A11'])

        # now work backwards to spot onto the agar
        for col in reversed(range(1, 13)):
            w = 'A' + str(col)
            aspirate_spot(p20_mult, p_dl[w], p_ag[w], spot_vol=1,
                          z_speed=50, spotting_dispense_rate=0.025)
        p20_mult.drop_tip()
